...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.dense2 = nn.Linear(in_features=1000, out_features=136)
    ...
    def forward(self, x):
        ...
        x = self.pool(self.activation1_5(self.conv2(x)))
        x = self.pool(self.activation1_5(self.conv3(x)))
        x = self.pool(self.activation1_5(self.conv4(x)))
        x = self.pool(self.activation1_5(self.conv5(x)))

        # Flatten
        x = x.view(x.size(0), -1)

        # Fully-connected layers
        x = self.activation1_5(self.dense1(x))
        x = self.dense2(x)
